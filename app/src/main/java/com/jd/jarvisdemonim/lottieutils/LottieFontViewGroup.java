package com.jd.jarvisdemonim.lottieutils;
...
public class LottieFontViewGroup extends FrameLayout {
     ...
     * @param composition
     */
    private long addComposition(LottieComposition composition) {
...
//        }
    ...
    public void setAnimDatas(String target) {
        final char[] chars = target.toCharArray();
        for (int i = 0; i < chars.length; i++) {
            final int finalI = i;
            String letter = "" + Character.toUpperCase((char) chars[finalI]);
            final String fileName = "Mobilo/" + letter + ".json";
            LogUtils.e("选中的文件:" + fileName + "/" + chars[finalI]);
            if (compositionMap.containsKey(fileName)) {
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
                        long l = addComposition(compositionMap.get(fileName));
                        delayms += l;
                    }
                }, delayms);
            } else {
                        ...
                        new OnCompositionLoadedListener() {
                            ...
                            public void onCompositionLoaded(final LottieComposition composition) {
                                ...
                                handler.postDelayed(new Runnable() {
                                    @Override
                                    public void run() {
                                        long l = addComposition(composition);
                                        delayms += 500;
                                        LogUtils.e("延迟的时间:-" + "fileName:" + delayms);
                                    }
                                }, delayms);
                            }
                        });
