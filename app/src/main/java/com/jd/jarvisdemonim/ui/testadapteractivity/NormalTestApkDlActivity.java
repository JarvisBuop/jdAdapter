 ...
 * Name:
 * OverView: //测试插件化启动未安装的apk;
 * Usage: 此为宿主程序;
 */

public class NormalTestApkDlActivity extends Activity {
    private static final String TAG = "ApkDlActivity";
    ...
    protected void initView(Bundle savedInstanceState) {
        mDexPath = getIntent().getStringExtra(EXTRA_DEX_PATH);//apk安装包的路径;
    ...
    protected void launchTargetActivity() {
        PackageInfo packageInfo = getPackageManager().getPackageArchiveInfo(
                mDexPath, PackageManager.GET_ACTIVITIES);//包中activity的信息;
        if ((packageInfo.activities != null)
                && (packageInfo.activities.length > 0)) {
            String activityName = packageInfo.activities[0].name;//第一个activity;
            mClass = activityName;
            launchTargetActivity(mClass);
        }
    }
