...
public class CrashUtils
    ...
    public boolean init() {
        ...
        if (Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            File baseCache = BaseConfig.application.getExternalCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        } else {
            File baseCache = BaseConfig.application.getCacheDir();
            if (baseCache == null) return false;
            crashDir = baseCache.getPath() + File.separator + "crash" + File.separator;
        }
        try {
            PackageInfo pi = BaseConfig.application.getPackageManager().getPackageInfo(BaseConfig.application.getPackageName(), 0);
