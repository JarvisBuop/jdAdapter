...
public class SpannableStringUtils {
    ...
    /*
    * 使用demo
    * msg 原信息;append()附加信息,样式处理;setClickSpann()
    * textView.setText(SpannableStringUtils.getBuilder(msg).append("处理信息").setClickSpan(clickableSpan).create());
    *  //注意下面这句必须写,否则点击事件不响应;
    *  textView.setMovementMethod(LinkMovementMethod.getInstance());//TextView多个地方响应点击事件;
    * ClickableSpan clickableSpan = new ClickableSpan() {
        ...
        public void updateDrawState(TextPaint ds) {
            ds.setColor(context.getResources().getColor(R.color.clickspan_color));
            ds.setUnderlineText(true);//设置下滑线;
        }
    };
    * */
    ...
    public static class Builder {
        ...
        private void setSpan() {
            ...
            if (imageIsBitmap || imageIsDrawable || imageIsUri || imageIsResourceId) {
                if (imageIsBitmap) {
                    mBuilder.setSpan(new ImageSpan(BaseConfig.application, bitmap), start, end, flag);
                    bitmap = null;
                    imageIsBitmap = false;
                    ...
                    mBuilder.setSpan(new ImageSpan(BaseConfig.application, uri), start, end, flag);
                    uri = null;
                    imageIsUri = false;
                } else {
                    mBuilder.setSpan(new ImageSpan(BaseConfig.application, resourceId), start, end, flag);
                    resourceId = 0;
