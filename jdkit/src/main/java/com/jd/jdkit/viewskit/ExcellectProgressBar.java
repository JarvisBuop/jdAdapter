...
/**
 */
public class ExcellectProgressBar extends ProgressBar {
    ...
    public ExcellectProgressBar(Context context) {
        this(context, null);
    }

    public ExcellectProgressBar(Context context, AttributeSet attrs) {
...
//        super(context, attrs);
    ...
    public ExcellectProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initDefaultAttrs(Context context) {
        ...
        mTextSize = ExcellectProgressBar.sp2px(context, 10);
        mTextMargin = ExcellectProgressBar.dp2px(context, 4);
        mReachedColor = Color.parseColor("#70A800");
        mReachedHeight = ExcellectProgressBar.dp2px(context, 2);
        mUnReachedColor = Color.parseColor("#CCCCCC");
        mUnReachedHeight = ExcellectProgressBar.dp2px(context, 1);
        mIsCapRounded = false;
        mIsHiddenText = false;

        mRadius = ExcellectProgressBar.dp2px(context, 16);
    }

    private void initCustomAttrs(Context context, AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.ExcellectProgressBar);
    ...
    protected void initAttr(int attr, TypedArray typedArray) {
        if (attr == R.styleable.ExcellectProgressBar_easy_pb_mode) {
            int ordinal = typedArray.getInt(attr, Mode.System.ordinal());
            mMode = Mode.values()[ordinal];
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_textColor) {
            mTextColor = typedArray.getColor(attr, mTextColor);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_textSize) {
            mTextSize = typedArray.getDimensionPixelOffset(attr, mTextSize);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_textMargin) {
            mTextMargin = typedArray.getDimensionPixelOffset(attr, mTextMargin);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_reachedColor) {
            mReachedColor = typedArray.getColor(attr, mReachedColor);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_reachedHeight) {
            mReachedHeight = typedArray.getDimensionPixelOffset(attr, mReachedHeight);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_unReachedColor) {
            mUnReachedColor = typedArray.getColor(attr, mUnReachedColor);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_unReachedHeight) {
            mUnReachedHeight = typedArray.getDimensionPixelOffset(attr, mUnReachedHeight);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_isCapRounded) {
            mIsCapRounded = typedArray.getBoolean(attr, mIsCapRounded);
            if (mIsCapRounded) {
                mPaint.setStrokeCap(Paint.Cap.ROUND);
            }
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_isHiddenText) {
            mIsHiddenText = typedArray.getBoolean(attr, mIsHiddenText);
        } else if (attr == R.styleable.ExcellectProgressBar_easy_pb_radius) {
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        } else if (mMode == Mode.Circle || mMode == Mode.CircleFull) {
...
//            super.onMeasure(widthMeasureSpec, heightMeasureSpec);
    ...
    protected synchronized void onDraw(Canvas canvas) {
        if (mMode == Mode.System) {
            ...
            onDrawCircle(canvas);
        } else if (mMode == Mode.CircleFull) {
            onDrawCircleFull(canvas);
        } else if (mMode == Mode.Comet) {
            // TODO
            super.onDraw(canvas);
        } else if (mMode == Mode.Wave) {
    ...
    private void onDrawWave(Canvas canvas) {
        ...
        canvas.drawCircle(mRadius, mRadius, mRadius + this.dp2px(getContext(), 2), mPaint);

        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setColor(mReachedColor);
        mPaint.setStrokeWidth(mReachedHeight);
        //画线;
        float angle = getProgress() * 1.0f / getMax();
        float angleStart = 0f;
        float angleEnd = 0f;
        Path path = new Path();
        if (angle >= 1) {
            ...
            float camber = Math.abs((float) Math.asin(angle * 2 - 1));//此为弧度;
            float as = (float) (180 * camber / Math.PI);
            angleStart = -as;
            angleEnd = 180 + 2 * as;
            path.lineTo((float) (mRadius * (1 + Math.cos(as))), (float) (mRadius * (1 - Math.sin(as))));
            path.arcTo(mArcRectF, angleStart, angleEnd, true);

            LogUtils.i("progrejj1", "start= " + angleStart + "end= " + angleEnd + "angle= " + angle + "\n" + "(" + mRadius * (1 + Math.cos(as)) + "," + (mRadius * (1 - Math.sin(as)) + ")"));
//            path.close();
            canvas.drawPath(path, mPaint);

        } else if (angle <= 0.5) {
            float camber = Math.abs((float) Math.asin(1 - angle * 2));
            angleStart = (float) (180 * camber / Math.PI);
            angleEnd = 180 - 2 * angleStart;
            path.lineTo((float) (mRadius * (1 + Math.cos(angleStart))), (float) (mRadius * (1 + Math.sin(angleStart))));
            path.arcTo(mArcRectF, angleStart, angleEnd, true);

            LogUtils.i("progrejj2", "start= " + angleStart + "end= " + angleEnd + "angle= " + angle + "\n" + "(" + mRadius * (1 + Math.cos(angleStart)) + "," + (mRadius * (1 + Math.sin(angleStart)) + ")"));
//            path.close();
            canvas.drawPath(path, mPaint);
        }
    ...
    private void onDrawHorizontal(Canvas canvas) {
        canvas.save();
        canvas.translate(getPaddingLeft(), getMeasuredHeight() / 2);//用于画在中间;
    ...
    private void onDrawCircleFull(Canvas canvas) {
        canvas.save();
        canvas.translate(getPaddingLeft() + mMaxStrokeWidth / 2, getPaddingTop() + mMaxStrokeWidth / 2);//中心

        mPaint.setStyle(Paint.Style.STROKE);
        mPaint.setColor(mUnReachedColor);
        mPaint.setStrokeWidth(mUnReachedHeight);
        canvas.drawCircle(mRadius, mRadius, mRadius + this.dp2px(getContext(), 3), mPaint);

        mPaint.setStyle(Paint.Style.FILL);
        mPaint.setColor(mReachedColor);
        mPaint.setStrokeWidth(mReachedHeight);
        float sweepAngle = getProgress() * 1.0f / getMax() * 360;
        canvas.drawArc(mArcRectF, -90, sweepAngle, true, mPaint);
