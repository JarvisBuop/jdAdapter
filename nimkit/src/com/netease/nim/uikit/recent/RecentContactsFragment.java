...
public class RecentContactsFragment extends TFragment implements TAdapterDelegate {
    ...
    //传入适配器的回调接口1,用于viewholder的类型总数;
    @Override
    public int getViewTypeCount() {
        return 2;
    }
    //传入适配器的回调接口2,用于标志位置几的item 是否可响应;
    @Override
    public boolean enabled(int position) {
        return true;
    }

    //重写刷新,判断是否有数据,没有数据则显示列表后的布局,显示无数据;
        ...
        listView.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                if (callback != null) {
                    ...
                    callback.onItemClick(recent);//回调点击事件;
                }
            }
        });
